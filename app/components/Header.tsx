...
export function Header() {
  return (
    <header className="relative z-50 bg-background">
      <span
        aria-hidden="true"
        className="absolute left-8 top-0 hidden h-40 w-px bg-border tablet:block desktop:left-20"
      />

      <div className="flex h-24 items-center px-8 tablet:h-40 tablet:px-[4.5rem] desktop:pl-[10.3125rem]">
        <Link
          href="/"
        ...
        >
          <Image
            ...
            height={40}
            priority
            className="h-auto w-[4.8125rem] desktop:w-[6.0625rem]"
          />
        </Link>

        <nav
          aria-label="Primary navigation"
          className="ml-[4.5rem] hidden tablet:block desktop:ml-24"
        >
          <ul className="flex items-center gap-12 desktop:gap-16">
            {navigation.map((item) => (
              <li key={item.href}>
        ...
        <details className="group ml-auto tablet:hidden">
            <summary
